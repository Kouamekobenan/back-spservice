  ...
  async receiveItems(id: string, data: ReceiveItemsDto, userId: string): Promise<PurchaseOrder> {
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        // Crée automatiquement une dépense pour les articles reçus
        const totalReceivedAmount = data.items.reduce((sum, receivedItem) => {
          const original = order.items.find(i => i.productId === receivedItem.productId);
          return original ? sum + receivedItem.quantityReceived * Number(original.unitCost) : sum;
        }, 0);

        if (totalReceivedAmount > 0) {
          const supplier = order.supplierId
            ? await tx.supplier.findUnique({ where: { id: order.supplierId }, select: { name: true } })
            : null;
          await tx.expense.create({
            data: {
              title: `Réception ${order.orderNumber}${supplier ? ` — ${supplier.name}` : ''}`,
              category: 'PURCHASE_ORDER',
              amount: totalReceivedAmount,
              date: new Date(),
              description: `Réception de ${data.items.length} article(s) — bon de commande ${order.orderNumber}`,
              shopId: order.shopId,
